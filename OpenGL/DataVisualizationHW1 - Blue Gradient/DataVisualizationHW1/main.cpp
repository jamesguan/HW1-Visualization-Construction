...
This generates the brain with blue because it represents fluids going throughthe brain
and adds a hint of green to emphasize the color changes to better
represent the data.
...
void readDataFile(char * file, SDL_Renderer * renderer) {
	...
	while (std::getline(infile, line)) {
		...
		int j = 0;
		while (iss >> value) {
			SDL_SetRenderDrawColor(renderer, 0, value / 3, value * 255, 255); // Renderer that relies on OpenGL
	...
	for (int i = 100; i < 100 + 255; i++) {
		SDL_SetRenderDrawColor(renderer, 0, (i - 100) / 3, i - 100, 255);
...
int main(int argc, char * argv[])
{
	...
	SDL_SetWindowTitle(window, "HW1: Diffusion Tensor Imaging - Blue");
